...
export async function POST(req) {
    ...
    try {
        ...
        const systemPrompt = ` You are an expert AI Chapters generator for JEE Advance, Mains and CET exam. Always respond in a JSON format with course title, overview, topics, and 20 - 25 words description.\n output format : { course_title : '...' \n, overview : '...'\n, chapters : [{chapter_number : 1, title : '...', topics : ['..','..']}, description : '...' \n, {...}\n ...]}`;
        const prompt = `Generate a comprehensive chapters for the concept "${concept}" for the exam "${exam}" at the "${level}" level. The roadmap should include an overview, chapters, and topics based on the daily study time of ${dailyTime} hours and a target completion time of ${targetTime} weeks. The roadmap should be ${difficulty?.difficulty} paced.`;
